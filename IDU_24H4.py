...
class Map:
    ...
    def recv(self, event: dict, player: Player):        # 接收游戏事件
            ...
            if self.dig_pos == (col, row):
                ...
                if dig_level_addition >= binfo[bid][3]:
                    ...
                    match dropbid:
                        ...
                        case 201201:
                            record.record('钻！石！')
                    current_area.append(Drop(dropbid))
        ...
        if event['mb 3']:
            match bid:
                case 0:
                    if chosen_slot.bid // 100000 == 2:      # 放置物品
                        if binfo[chosen_slot.bid][1]:
                            self.data[row][col] = binfo[chosen_slot.bid][1]
                            chosen_slot.count -= 1
                            chosen_slot.update()
                            match self.data[row][col]:     # 放置功能/特殊方块
                                case 100601:
                                ...
                                case 101501:
                                    ...
                                    self.function_blocks[(col, row)] = pg
                        elif chosen_slot.bid // 10000 == 26 and not player.full:     # 食物，吃
                            player.eat(*binfo[chosen_slot.bid][3:])
                            chosen_slot.count -= 1
                            chosen_slot.update()
    ...
    def update(self, visitor: Entity | Player):
        ...
        for index in range(9):
            ...
            while p < len(update_range[index]):
                ...
                if tp is Drop:
                    ...
                    drop.move(self)
                    drop.rub_ground()
                    if self.update_entity_aa(each, update_range[index]):           # 移动后要更新区块
                        continue
                    drop.show(self)
                    # 获取掉落物的更新
        ...
        if time.time() - self.last_born_friendly_mob_time > self.last_born_friendly_mob_interval:
            ...
            if animal_count < 30:
                # 羊
                offsetx = random.choice([random.randint(-1800, 800), random.randint(800, 1800)])
                ...
                for i in range(random.randint(1, 2)):
                    ...
                    x = min(max(0, player.x + offsetx + detail_offsetx), MPW_px - 1 - 120)
                    y = min(max(0, player.y + offsety), MPH_px - 1 - 30)
                    cow = AutoAnimal('cow', 85, 0, 135, 60, x, y, (102, 77, 48))
                    map.get_aa(cow.x, cow.y).append(cow)
        visitor.next(self)
    ...
    def show_light(self):
        h, w = SCH_bl + 1, SCW_bl + 1
        lights = []
        for row in range(h):
            for col in range(w):
                bid = self.data[self.y_bl + row][self.x_bl + col]
                match bid:
                    case 0 if (row == h - 1 or self.data[self.y_bl + row + 1][self.x_bl + col]):
                        lights.append((col, row, 1))
                    case 101901:
                        lights.append((col, row, 1.3))
        ...
        for row in range(h):
            for col in range(w):
                ...
                if bid:
                    light_value = 0
                    for l in lights:
                        light_value = min(1.3, max(light_value, l[2] * (1 - min(1, get_distance(l[0], l[1], col, row) / 10))))
                    image = binfo[bid][0]
                    image = pygame.transform.hsl(image, 0, 0, light_value - 1)
                    screen.blit(image, (col * BLOCK_SIZE - self.x_offset, row * BLOCK_SIZE - self.y_offset))
            ...
            if other.bid:
                ...
                match other.bid:
                    case 201301:
                        record.record('来硬的')
                    case 201401:
                        record.record('金！色！传！说！')
                    case 300101:
                        record.record('进击的镐子I')
                    case 300201:
                        record.record('进击的镐子II')
                    case 300301:
                        record.record('进击的镐子III')
                    case 300401:
                        record.record('进击的镐子IV')
                    case 300501:
                        record.record('进击的镐子V')
                    case 302101:
                        record.record('原始武装(锐剑I)')
                    case 302201:
                        record.record('坚如磐石(锐剑II)')
                    case 302301:
                        record.record('铁血战士(锐剑III)')
                    case 302401:
                        record.record('黄金之刃(锐剑IV)')
                    case 302501:
                        record.record('钻石耀剑(锐剑V)')
            res = 'complete'
        self.update()
        return res
...
class AutoAnimal(Animal):
    def __init__(self, name: str, hp: int | float, attack: int | float, w: int, h: int, x: int | float = 0, y: int | float = 0, color = (0, 255, 0)):
        ...
        if event_state['mb 1'] and collision(*self.rect, *event_state['mpos'], 1, 1):
            if time.time() - player.last_attack_time > player.attack_interval:
                ...
                if enemy_handin.bid // 100000 == 3:
                    total_attack += binfo[enemy_handin.bid][2][3]
                    if enemy_handin.durable:
                        enemy_handin.durable -= 1
                        enemy_handin.update()
                if player.vy > 2:
                    total_attack *= 1 + math.sqrt(player.vy - 2) / 2
                self.hp -= total_attack
    ...
    def draw(self, x: int | float, y: int | float):
        ...
        draw_progress(screen, self.hp, self.maxhp, self.rect.x, self.rect.y - 3, self.w, 4, (255, 0, 0), (0, 0, 0))
...
class Player(Animal):
    ...
    def eat(self, saturation: int | float, hunger: int | float):
        self.saturation = min(self.saturation + saturation, 150)
        self.hunger = max(self.hunger - hunger, 0)
    def heal(self):
        if self.saturation > 20 and self.hp < 95:
            self.hp += 0.05
            self.saturation -= 0.1
    def next(self, map: Map):
        self.show(map)
    ...
    @property
    def full(self):
        return self.hunger == 0 and self.saturation >= 80
...
chatbox = Chatbox(save.chat_data, save.chat_conn, save.chat_images, save.chat_hides)
chatbox.set_mode(200, 60, 600, 100)
chatbox.next('0')
...
while True:
    ...
    show_text(screen, f'FPS: {clock.get_fps():.1f}', (255, 0, 0), (10, 10), 20)
